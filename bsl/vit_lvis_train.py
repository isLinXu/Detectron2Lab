...
for data in dataset_ann_list:
    ...
    if len(data['annotations']) > 0 and os.path.exists(data['file_name']) == True and \
        ...
        for d in data['annotations']:
            ...
            if data_json['height'] >= 100 and data_json['width'] >= 100:
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if img_cropped.shape[0] > 0 and img_cropped is not None and w >= 100 and h >= 100:
            ...
            # cv2.waitKey(0)
            cv2.imwrite('/media/admin1/mobileSSD/datasets/lvis_clas_image6/' + str(data['category_id']) + "_" + str(
...
# models = models.vision_transformer.vit_b_16
# model = models.vit_b_16(pretrained=True, num_classes=1203)

# model = models.vit_b_16(pretrained=True)
# weights = torchvision.models.ViT_B_16_Weights.DEFAULT
# model.load_state_dict(weights['model_state_dict'])
# model.head = torch.nn.Linear(768, 1203)
model = timm.create_model("vit_base_patch16_224", pretrained=True)
num_ftrs = model.head.in_features
model.head = nn.Linear(num_ftrs, 1203)
...
for epoch in range(num_epochs):
    ...
    if val_acc > best_acc:
        best_acc = val_acc
        best_model_weights = model.state_dict()
        torch.save(best_model_weights, '../ckpts/vit_best_model.pth')
